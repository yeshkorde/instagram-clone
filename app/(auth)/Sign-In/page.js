...
function Page() {
  ...
  const onsubmit = async(data)=>{
    try {
     setisLoding(true)
      reset();
      const responce = await axios.post('api/auth/sign-in',data)
    ...
    <div className="w-full h-full px-4 mt-2">
     <form action="" className="h-full w-full flex flex-col gap-10" onSubmit={handleSubmit(onsubmit)}>
      <div className="w-full">
        <label htmlFor="email" className="text-[12px] ml-4 font-semibold">Email</label>
        <input 
        type="email"
      ...
      <div className="w-full">
        <label htmlFor="password" className="text-[12px] ml-4 font-semibold">Password</label>
        <input
       ...
       </div>
      {
        ...
        :<div>
          ...
          <button className="py-4 w-full bg-gradient-to-tr from-yellow-500 via-pink-500 to-purple-700 text-white border-4 border-white font-bold rounded-2xl shadow-2xl">Sign-In</button>
          <Link href={"/Sign-Up"}><p className="text-[12px] font-semibold mt-2 ml-4 ">You All Ready have Account Then <span className="text-blue-500">Sign-Up</span></p></Link>
          </div>
      }
     </form>
    </div>
